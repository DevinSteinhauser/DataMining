...
Going further, a contingency table can show items that are purchased together and have a high lift value.
```{r}
# Products that are purchased together and have a high lift value
crossTable(groceries, measure='lift', sort=T)[1:5, 1:5]
```

It makes sense that the most frequently purchased items also are frequently purchased together. Soda and vegetables have a lift value lower than one, which could potentially be explained by similar health characteristics. Whole milk and soda also has a lift value lower than one, which may also seem intuitive as they are substitutes for one another.


A more useful method for mining association rules is applying the Apriori Principle, which is constructed below. In order to search for interesting relationships, we will generate the two-step approach of first finding the most frequently purchased baskets and then generating high confidence rules from each frequent itemset.

```{r}
transactions <- apriori(groceries, parameter=list(support=0.01, confidence=0.1, target='frequent'))
summary(transactions)
quality(transactions)$lift <- interestMeasure(transactions, measure='lift', groceries)
inspect(sort(transactions, by='lift', decreasing = T)[1:10])
```

The basket with the highest estimated lift is citrus fruit, root vegetables and other vegetables. This is intuitive as all of these items are near one another in the produce section of the grocery store. The basket of curd, whole milk and yogurt follow the same pattern as they are all dairy products.

Calculating the frequent itemsets is still computationally expensive, so it will be helpful to reduce the number of candidates by pruning out infrequent baskets.

```{r, warning = FALSE}

rules <- apriori(groceries, parameter=list(support=0.01, confidence=0.1, target='rules'))
summary(rules)

plotly_arules(rules, measure=c("support", "lift"), shading="confidence", jitter=0) %>% layout(title = "Generated Rules Plot")
```

You can set support and confidence thresholds to the dataset depending on certain measurements you wish to meet. After plotting the generated rules, it's shown that a majority of the baskets have a support level of approximately 0.01 and a lower confidence level estimated around 0.1-0.2. Thus, those are the thresholds that will continued to be used for the next couple of examples.
...
Now that the Apriori Principle has been implemented, we are able to search for interesting relationships between the variables. Below are several examples of inspecting the rules in order to view these association rules for the shopping baskets. We will begin by examining two common relationships from frequently bought items and then examine three more interesting examples.
```{r}
# Finding Complements
arules::inspect(subset(rules, subset=items %ain% c("whole milk", "yogurt") & confidence>0.5))
```
It would make intuitive sense for whole milk and yogurt to be complements as they are both dairy products, which is shown from the high estimated confidence and lift values.
```{r}
# Finding Substitutes
arules::inspect(subset(rules, subset=lhs %pin% "whole milk" & rhs %in% "soda" & confidence>0.1))
```
Whole milk and soda could be considered subsitutes as they are both beverages, and this is shown from the low estimated confidence and lift values.

It's possible to find association mining rules of items purchased either before or after a certain item. At an attempt of finding more interesting relationships, the support and confidence thresholds have been lowered to 0.001 to 0.01 for the following rules. While these thresholds are lowered, the relationships below show higher lift values, showing that the purchased items happen more often than would be expected.
```{r}

liquorrules <-apriori(data=groceries, parameter=list (supp=0.001, conf=0.01), appearance = list(lhs="liquor"))
liquorrules_conf <- sort(liquorrules, by='lift', decreasing=TRUE)
arules::inspect(head(liquorrules_conf))
liquorrules2 <-head(liquorrules, n=10, by="lift")
plot(liquorrules2, method="graph", engine="htmlwidget")
```

Here, it was tested what customers decided to buy after purchasing liquor. The relationships make sense, as red/blush wine, bottled beer, shopping bags, and soda could all be considered complements and were purchased after liquor. In addition, the lift values are all above one.
```{r}
bakingrules <-apriori(data=groceries, parameter=list (supp=0.001, conf=0.01), appearance=list(lhs=c("baking powder", "sugar")))
bakingrules_conf <- sort(bakingrules, by='lift', decreasing=TRUE)
arules::inspect(head(bakingrules_conf))
bakingrules2 <-head(bakingrules, n=10, by="lift")
plot(bakingrules2, method="graph", engine="htmlwidget")
```

This is a similar process to the liquor rules determined above, but with baking materials. The six relationships show flour, margarine, whipped and sour cream, and salt were all purchased with baking powder and sugar. The lift values are above five for each basket.
```{r}
frozenpotatorules <-apriori(data=groceries, parameter=list (supp=0.001, conf=0.01), appearance=list(lhs="frozen potato products"))
frozenpotatorules_conf <- sort(frozenpotatorules, by='lift', decreasing=TRUE)
arules::inspect(head(frozenpotatorules_conf))
frozenpotatorules2 <-head(frozenpotatorules, n=10, by="lift")
plot(frozenpotatorules2, method="graph", engine="htmlwidget")
```

These results are slightly less straightforward than the rules above. For instance, white bread, frankfurter, frozen vegetables, and pork could be seen as complements to frozen potatoes. Chocolate and fruit and vegetable juice are items that aren't usually associated with frozen potatoes, though their occurrence is more frequent than would be expected given their lift values.